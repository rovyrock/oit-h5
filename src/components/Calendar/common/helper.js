...
export default function generateCalendarGroup(options) {
    return function (generateDate, selectedDate, isShowStatus) {
        ...
        const { format, startFromMonday, singleMonth } = options;
        ...
        if (startFromMonday) {
            prevMonthDaysLeft = firstDate.day() === 0 ? 6 : firstDate.day() - 1;
        }
        else {
            prevMonthDaysLeft = firstDate.day() === 0 ? 0 : firstDate.day();
        }

        // 生成上个月的日期
        for (let i = 1; i < prevMonthDaysLeft + 1; i++) {
